      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {APPLICATION_TYPE_CONFIGS.map((config) => {
            ...
            return (
              <div
                key={config.type}                data-testid={`application-type-${config.type.toLowerCase().replace('_', '-')}`}
                className={`
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {selectedConfig.targetCountries.map((country) => {
              ...
              return (
                <button
                  key={country}                  data-testid={`target-country-${country.toLowerCase().replace(/\\s+/g, '-')}`}
        ...
        <div>
          ...
          <div className="space-y-3">
            {[
              ...
              return (
                <label
                  key={option.value}                  data-testid={`processing-priority-${option.value}`}
                  className={`
